...
void app_init(void)
{
  ...
  sc = sl_pwm_init(&sl_pwm_led0, &pwm_led0_config);
  app_assert_status_f(sc, "sl_pwm_init failed");
  ...
  for (uint8_t i = 0u; i < CS_INITIATOR_MAX_CONNECTIONS; i++) {
    if (cs_initiator_instances[i].measurement_arrived) {
      ...
      if (rtl_config.algo_mode == SL_RTL_CS_ALGO_MODE_REAL_TIME_FAST
        ...
        log_info(APP_INSTANCE_PREFIX "Velocity: %d.%02u" NL,
                 cs_initiator_instances[i].conn_handle,
                 ((int)cs_initiator_instances[i].measurement_mainmode.velocity),
                 (uint16_t)((uint32_t)(ABS(cs_initiator_instances[i].measurement_mainmode.velocity) * 100.f)) % 100);
      }
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_system_boot_id:
    {
      ...
      sc = sl_bt_system_set_tx_power(min_tx_power_x10,
                                     ...
                                     &max_tx_power_x10);
      if (sc != SL_STATUS_OK) {
        log_error(APP_INSTANCE_PREFIX "sl_bt_system_set_tx_power failed" NL, sc);
      }
    ...
    case sl_bt_evt_connection_opened_id:
      ...
      if(ble_peer_manager_is_conn_handle_in_array(evt->data.evt_connection_opened.connection)) {
        ...
        log_info(APP_INSTANCE_PREFIX "Connection opened with Provisioner"
                                     " '%02X:%02X:%02X:%02X:%02X:%02X'" NL,
                 evt->data.evt_connection_opened.connection,
                 evt->data.evt_connection_opened.address.addr[5],
                 evt->data.evt_connection_opened.address.addr[4],
    ...
    case sl_bt_evt_connection_closed_id:
      ...
      if(ble_peer_manager_is_conn_handle_in_array(evt->data.evt_connection_closed.connection)) {
        ...
        if ((sc == SL_STATUS_NOT_FOUND) || (sc == SL_STATUS_INVALID_HANDLE)) {
          ...
          app_assert_status(sc);
          log_info(APP_INSTANCE_PREFIX "Initiator instance removed" NL, evt->data.evt_connection_closed.connection);
        }
        delete_initiator_instance(evt->data.evt_connection_closed.connection);
        sc = ble_peer_manager_delete_connection(evt->data.evt_connection_closed.connection);
        if (sc != SL_STATUS_OK) {
          log_error(APP_INSTANCE_PREFIX "Peer Manager connection removal failed" NL, sc);
        }
      }
      log_info(APP_INSTANCE_PREFIX "Connection closed" NL, evt->data.evt_connection_closed.connection);
    ...
    case sl_bt_evt_scanner_legacy_advertisement_report_id:
      ...
      if (match) {
        ...
        uint8_t reflector_connection_id;
        sc = sl_bt_connection_open(evt->data.evt_scanner_legacy_advertisement_report.address,
                                   evt->data.evt_scanner_legacy_advertisement_report.address_type,
                                   sl_bt_gap_phy_1m,
                                   &reflector_connection_id);

        if (sc != SL_STATUS_OK) {
          log_error(APP_INSTANCE_PREFIX "Reflector connection failed" NL, sc);
        } else {
          sc = ble_peer_manager_add_connection(reflector_connection_id,
                                               evt->data.evt_scanner_legacy_advertisement_report.address,
                                               sl_bt_connection_role_peripheral);
          if (sc != SL_STATUS_OK) {
            log_error(APP_INSTANCE_PREFIX "Peer Manager connection adding failed" NL, sc);
          }
        }
      }
      break;
    ...
    case sl_bt_evt_scanner_extended_advertisement_report_id:
      ...
      if (match) {
        ...
        uint8_t reflector_connection_id;
        sc = sl_bt_connection_open(evt->data.evt_scanner_extended_advertisement_report.address,
                                   evt->data.evt_scanner_extended_advertisement_report.address_type,
                                   sl_bt_gap_phy_1m,
                                   &reflector_connection_id);
    ...
    case sl_bt_evt_system_resource_exhausted_id:
      log_error(APP_INSTANCE_PREFIX "System resources exhausted!" NL, sc);
      break;

    // CS events and GATT Characteristic
...
void sl_btmesh_on_event(sl_btmesh_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_btmesh_evt_node_initialized_id: {
      log_info(APP_PREFIX "sl_btmesh_evt_node_initialized_id" NL);
    ...
    case sl_btmesh_evt_node_provisioned_id: {
      log_info(APP_PREFIX "sl_btmesh_evt_node_provisioned_id" NL);
    } break;
    case sl_btmesh_evt_proxy_connected_id: {
      log_info(APP_PREFIX "sl_btmesh_evt_proxy_connected_id" NL);
    } break;
